...
update employees set designation="Data
Scientist" where designation is null;
select * from employees order by department_id , salary desc;
select * from employees where hire_date >='2018-01-01' and hire_date <='2018-12-31' limit 5;
select sum(salary) as total_salary from employees e join departments d on e.department_id=d.department_id where d.department_name="finance";
select min(age) from employees;
select l.location,max(e.salary)  from employees e join location l on e.location_id=l.location_id group by l.location;
...
select l.location, avg(age) from employees e join location l on e.location_id=l.location_id where e.gender='f' group by l.location having avg(age)<30;
...
select d.department_name,count(e.employee_id) as total_employees from  departments d left join employees e on e.department_id=d.department_id
 group by d.department_name;
 select l.location,e.employee_name from employees e right join location l on e.location_id=l.location_id;
 
 /*just for query checking*/
 
 insert into location(location) values("thrissur");
